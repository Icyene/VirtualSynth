...
public class KeyboardControl extends JPanel {
    private static final Font KEY_FONT = new Font("Segoe UI Symbol", Font.BOLD, 18);
        ...
        addMouseListener(new MouseAdapter() {
            ...
            public void mousePressed(MouseEvent e) {
                for (int i = 0; i != KEY_BOUNDS.length; i++) {
                    if (KEY_BOUNDS[i].contains(e.getPoint()) && !selected[i]) {
            ...
            public void keyReleased(KeyEvent e) {
                for (int i = 0; i != KEYS; i++) {
                    for (int c = 0; c != KEY_BINDINGS[i].length; c++) {
                        if (e.getKeyCode() == KEY_BINDINGS[i][c]) {
                            selected[i] = false;
